...
# Rename deploy name
for f in $(find . -name '*.yaml' -or -name '*.gotmpl'); do gsed -i "s|julienne|$CALL|g" $f; done >/dev/null && echo " Rename namespace, domens, manifests"
...
ssh root@$CLUSTER_HOST "minikube kubectl -- create namespace $CALL" >/dev/null && echo "󰣀 Create namespace";
ssh root@$CLUSTER_HOST "minikube kubectl -- apply -f pv.yaml" >/dev/null && echo "󰣀 Create PV";
ssh root@$CLUSTER_HOST "minikube kubectl -- apply -f pvc.yaml -n $CALL" >/dev/null && echo "󰣀 Create PVC";
